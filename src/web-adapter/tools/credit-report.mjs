...
console.log('\n总任务数: ' + nAll + ' (接口 total=' + l.total + ')');
console.log('计费任务 paid=true : ' + paidAll + ' 条');
console.log('免费任务 paid=false: ' + (nAll - paidAll) + ' 条');
console.log('credits 字段合计    : ' + totAll + '  ← 注意与 paid 反相，不代表花费');
console.log('本次自动化测试      : ' + nMine + ' 条, credits 合计 ' + totMine + ', paid=true ' + paidMine + ' 条');

// 权威余额：credits.getCredits 的 totalRemaining（与官方 API currentBalance 同池）
let balance = null;
try {
  balance = await c.getCredits();
  console.log('\n实时余额 (credits.getCredits → totalRemaining): ' + balance);
} catch (e) {
  console.log('\n余额查询失败: ' + String(e.message).slice(0, 100));
}

fs.writeFileSync('/tmp/credits.json', JSON.stringify({
  totAll, nAll, paidAll, freeAll: nAll - paidAll, totMine, nMine, paidMine, balance,
}, null, 2));
