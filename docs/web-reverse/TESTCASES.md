...
**另接受 `WxH` 写法**（2026-09-18 补；实现与门禁见 `tests/test_ratio_normalize.py`）：

| 值 | 结果 |
| --- | --- |
| `1920x1080`（约分后**在**枚举里） | → `16:9`，留痕 |
| `1024x1792`（约成 `4:7`，**不在**枚举里） | **就近吸附** → `9:16`，留痕（输出比例与请求不同） |
| `5:4`、`abc` 等认不出的形态 | **仍然 400** —— 兜底不是"什么都收" |

⚠️ 上面 7 种是 **Ark 文档**的枚举；站点 UI（= minimaxH3 真实上游）只给 `16:9` / `9:16` /
`1:1`（抓包 JS 模块 85743）。`4:3` `3:4` `21:9` 在 H3 上**从未真实提交验证过**。
