...
**另接受 `WxH` 与 `W:H` 两种写法**（2026-09-18 补；实现与门禁见
`tests/test_ratio_normalize.py`）—— 用户口径"等比 或者按比例 传都可以"：

| 值 | 结果 |
| --- | --- |
| `1920x1080`（约分后**在**枚举里） | → `16:9`，留痕 |
| `1024x1792`（约成 `4:7`，**不在**枚举里） | **就近吸附** → `9:16`，留痕（偏差 1.6%） |
| `5:4` / `4:7` / `2.35:1`（比例串，**两线都认**） | 就近吸附 → `4:3` / `9:16` / `21:9`，留痕 |
| `abc` / `16/9` / `0x100`（连形态都认不出） | **仍然 400** —— 兜底不是"什么都收" |

🔴 **吸附永不拒绝**（2026-09-18 用户口径"能提交成功就行"）：偏差再大也只吸附 + 留痕
（`3000x1000` → `21:9`，偏差 28.6%）；越过 `RATIO_SNAP_WARN_DRIFT`（15%）只是**告警升级
措辞**并给出可替换写法（如 `2520x1080`），不是拒绝。⚠️ 曾按"偏差 >15% 就 400"实现过
一轮，被该口径推翻 —— 别改回去（`TestSnapIsNeverRejected` 守着）。

**`/v1/videos` 面（`size` 字段）的唯一差别**（2026-09-18 起）：走**同一份**归一化，但传
`fallback="16:9"` ⇒ 上面那类**认不出的值兜底 `16:9`**（不 400）；比例串与 `WxH` 两线
一致。两条线共用函数、差别只有这一个参数（`fallback=None` = 方舟线），由
`tests/test_ratio_normalize.TestTwoLinesDifferOnlyByFallback` 逐值锁死。理由：该端点的调用方
是 OpenAI SDK 用户，`size` 写错一个字符就整请求失败，他能拿到的信息只有"请求失败了"，
代价远大于拿到一档相近的画面；兜底**照样留痕**，所以没有退化成静默改写。

⚠️ **更正（2026-09-18，站点 UI 截图取证）**：本段原写"站点 UI（= minimaxH3 真实上游）只给
`16:9` / `9:16` / `1:1`（抓包 JS 模块 85743），`4:3` `3:4` `21:9` 在 H3 上从未真实提交
验证过"。**截图显示站点 `Aspect Ratio` 选择器就是六档** —— `21:9` / `16:9` / `4:3` /
`1:1` / `3:4` / `9:16`（`16:9` 默认选中），与代码里的 `RATIO_ORDER` 逐项一致。旧结论只
反映"在 JS 模块 85743 里翻到的那三个"，**不等于 UI 全集**。
✅ **已闭环（2026-09-18 真实提交实测，`/v1/videos` 480p + pro 凭据，全部 `billed=False`）**：
六档里此前从未提交过的三档**都能出片且比例精确**，判据是**成片实际像素宽高比**：

| `ratio` 入参 | 成片分辨率 | 实测比例 | 结论 |
| --- | --- | --- | --- |
| `4:3` | `640x480` | 1.333 | 上游确实支持 |
| `3:4` | `480x640` | 0.750 | 上游确实支持 |
| `21:9` | `1120x480` | 2.333 | 上游确实支持 |
| `16:9`（`size="banana"` 兜底而来） | `864x480` | 1.800 | 兜底链路真实生效 |

⇒ 「UI 里可选」＝「上游确实能出」，六档均为真实档位。
⚠️ 另：**水印**与档位无关但与账号等级有关 —— 站点 FAQ（`captured/home.html` 原文）明写
"free plan 出片**带水印**，Premium/Pro 才无水印"；本次只在 **pro 账号**上实测到"无水印"
这一面（5 条成片逐帧确认），free 账号因槽位被占**未取到样本**。取证与时间线见项目日志
