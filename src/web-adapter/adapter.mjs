...
import { AvmClient, imageDimensions, normalizeCookieHeader } from './client.mjs';
...
async function loadCookie() {
  // 裸 token 也接受（自动补 auth_session= 前缀并告警）。AvmClient 里也会兜一次，
  // 但在这里规范意味着本函数返回的值**永远**是可直接用的 Cookie 头。
  if (process.env.AVM_COOKIE) return normalizeCookieHeader(process.env.AVM_COOKIE);
