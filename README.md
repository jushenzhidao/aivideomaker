...
- 免费组合：**480p / 5s / turbo**。
- **时长约束按分辨率而异**：`720p` / `1080p` 是**连续秒数**（`5–20`，仅越界时才就近钳制）；`480p` 是**离散档位 `5 / 10 / 15 / 20`**（站点实测），非档位值**就近吸附并留痕**。
...
>
> **实测补充（2026-09-17）—— 账号数多时，单实例 `passthrough` 优于多实例部署。**
> 上面两条路都成立，但规模成本差很远：多实例是「一号一实例」（100 号 = 100 个实例，运维量随
> 号数线性增长）；而单实例 `passthrough` 让每个凭据各自持有独立上游与闸门，
> **总并发 = 4 × 号数**，一个实例即可承载 —— 实测容器在 **单 worker + 放开 fd** 后接待
> 1000 / 2000 / 4000 并发**全部成功**（约 200 req/s），100 号 = 400 并发余量充足。
> ⚠️ 两条铁律：① **同一个账号绝不能同时被两个实例使用** —— 闸门是进程内的、两实例互不可见，
> 合计会超该账号额度 ⇒ 该号提交**全 502**（实测：停掉第二个实例后同一账号立刻恢复 200）；
> 且**本实例的任务表看不出异常**（显示在飞 0，占用方在另一个实例里）。多实例只能按**账号**
> 划分号集合，不能按请求轮询。② 要「每号打满 4 并发」，`AVM_WORKERS` 必须为 **1**
> （实测依据见下文「多 worker 是这里唯一的坑」）。
...
**实测补充（2026-09-17，E2E-AVM-017 / 018）—— 分摊只保证「不超」，不保证「打得满」。**
上面的 `÷N` 让全局额度不超上游，但额度被切成 N 份后，同一账号的多个请求若落到同一个 worker，
超出该 worker 那 1 个槽的就得阻塞等槽（`acquire` 最长 600s，客户端通常先超时）。同一场景实测
（并发 8 / 2 个账号 / 目标每号 4 并发）：

- `AVM_WORKERS=1` ⇒ 槽位集中的账号 **4/4 全成功**（这是「每号打满」的唯一配置）
- `AVM_WORKERS=4` ⇒ 仅 **2/8** 成功，其余客户端 `TimeoutError`

⇒ 「每 worker 满额度」与「多 worker」在配额守恒下**互斥**：要每号打满，worker 必须为 1；
多 worker 只适合「每号只占 1 个并发」的轮询式调度（每 worker 至少有 1 槽，请求落哪都够）。
若确实要多 worker 的接待/隔离能力，唯一可行形态是**多实例 + 前置代理按账号粘性分流**
（每实例仍 `WORKERS=1`），而不是在一个容器里加 worker。
