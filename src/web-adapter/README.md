...
**结论：`auth_session` 于 `2027-10-15T03:41:53Z` 过期，TTL 400 天，站点不做滚动续期。**

### 过期时间从哪来

浏览器导出的 **cookie jar**（`cookies.json`）里带 `expirationDate` 字段：

```json
{ "name": "auth_session", "value": "n557e2…3bir", "expirationDate": 1823571713,
  "httpOnly": true, "secure": true, "sameSite": "lax" }
```

| 项 | 值 |
| --- | --- |
| 绝对过期 | **`2027-10-15T03:41:53Z`**（本地 2027-10-15 11:41:53，UTC+8） |
| 登录时刻（≈订阅开始 `2026-09-10T03:50:58Z`） | 2026-09-10 |
| **TTL** | **400.0 天** |

> 注意区分两条通路：`document.cookie` 读不到属性，**导出文件里却包含属性**。
> 早先"读不到过期时间"的判断是错的 —— 数据一直在 jar 里。

### 为什么不能靠解析或接口取

| 检查项 | 结果 |
| --- | --- |
| token 形态 | 40 字符不透明随机串（`^[a-z0-9]{40}$`），**不是 JWT，不含时间戳** |
...
| 服务端 `UserSession.expiresAt` | 数据库字段，**未通过任何接口暴露** |

因此：**token 解析不出来、接口查不到、响应头也不重下发** —— 唯一可靠来源是登录时下发的
`Set-Cookie`，而导出的 cookie jar 恰好保留了它。

**别再用 Auth.js 默认的 30 天来推断**：默认值只是量级参考，本站实测 TTL 是 400 天。

### 监控（兜底）

```bash
AVM_COOKIE='...' node tools/check-session.mjs
node tools/session-diagnose.mjs                    # 匿名对照（不带 cookie，零成本）
AVM_COOKIE='...' node tools/session-diagnose.mjs   # 有凭据：身份 + 闸门 + 余额
node tools/session-diagnose.mjs --dry-run          # 离线自检，完全不联网
```

`session-diagnose.mjs` 的三条探针都是**只读 query**（`auth.user` / `model.needsCaptcha` /
`credits.getCredits`），**不创建任务、不消耗积分**；不带 cookie 时即匿名对照。
只有 `--submit` 会发一次提交，且请求体**硬编码**为免费组合（turbo / 480p / 5s），
故意不可参数化。`--dry-run` 下脚本**不输出任何结论** —— 没问过服务端就不该下断言。

**`AVM_COOKIE` 的格式（高频误解）**：它是**整条 `Cookie` 请求头**，会被**逐字**发出，
不是裸 token。最小正确写法就是 `auth_session=<40位>`（`NEXT_LOCALE=zh` 非必需）。
只传裸 `<40位>` 会被 `normalizeCookieHeader()` 自动补全并打印告警，但**别依赖这个兜底**；
`npm run test:unit` 覆盖了这条契约。

> **完整 runbook**（凭据落位、到期排期、匿名路径判定标准、计费护栏）见
> [`docs/web-reverse/session-runbook.md`](../../docs/web-reverse/session-runbook.md)。

校验会话并把 `first_seen` / `last_ok` 记到 `.session-state.json`，失效时退出码非 0，
可直接挂到定时任务里做告警。换新 cookie 时指纹变化会自动重置计时。

> 相关背景（tRPC 机制、Prisma 数据模型、积分池口径）见
> [`docs/web-reverse/README.md`](../../docs/web-reverse/README.md) 与
> [`cookies.md`](../../docs/web-reverse/cookies.md)。

> 给自动化的建议：**不要假设它能活多久**。用 `check-session.mjs` 定期探活，
> 失效即告警；并**照 runbook §3.1 手工重导 cookie** —— 必须用浏览器扩展导出
> （`document.cookie` 拿不到 `expirationDate`），覆盖后**核验指纹已变化**。
> 完整步骤见 [`docs/web-reverse/session-runbook.md`](../../docs/web-reverse/session-runbook.md) §3.1。
