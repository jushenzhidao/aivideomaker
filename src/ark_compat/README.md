...
`WxH`（`1920x1080`）与比例串（`5:4`、`2.35:1`）同样**就近吸附 + 必留痕**；
**认不出的值兜底 `16:9`，不再让整请求 400**（2026-09-18 起 —— 本端点的调用方是 OpenAI
SDK 用户，能拿到的信息只有"请求失败了"，改尺寸重试的代价远大于拿到一档相近的画面）。
方舟线**不随本端点放宽**，认不出仍 400（见下方 `ratio` 行）。省略 `size` 仍是"没指定"
（交给上游默认），**不是**兜底 —— 两者语义不同，别混。
`first_frame_image` / `last_frame_image` → 首/尾帧
...
| `ratio` | → `aspectRatio`（站点字段名）；`adaptive` 表示跟随源图，不设值。**另接受两种写法**（`WxH` 如 `1920x1080` / `1024x1792`，与 `W:H` 比例串如 `5:4` / `2.35:1`）：约分后正好在枚举里就用它，约不进则**就近吸附**并**必留告警**（含偏差百分比）。🔴 **吸附永不拒绝**（"能提交成功就行"）：偏差再大也不 400（`3000x1000` → `21:9`，偏差 28.6%），越过 15% 只**升级措辞**并给可替换写法 —— 曾按"超 15% 就 400"实现过一轮，已被推翻。连形态都认不出的（`abc`、`16/9`）**仍然 400** —— `/v1/videos` 面的 `size` 走同一份归一化但传 `fallback="16:9"`，故那边会兜底而不报错；两条线共用函数、差别只有这一个参数（`tests/test_ratio_normalize.TestTwoLinesDifferOnlyByFallback` 锁死）。吸附目标只有站点 UI 的六档（`21:9` / `16:9` / `4:3` / `1:1` / `3:4` / `9:16`）；`requested.ratio` 回显调用方**原值**，改写结果看 `effective.aspectRatio` |
