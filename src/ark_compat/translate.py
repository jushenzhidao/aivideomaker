...
# 🔴 2026-09-24 **已实施：`480p` 改回离散表**（真实提交实测 + 生产实锤双重驱动）
#
#   站点对 **480p 只收离散的 `5 / 10 / 15 / 20`**。站点原话（真实提交被拒时返回）：
#       `ai.minimaxH3: 480p supports 5s, 10s, 15s, or 20s duration.`
#   依据链：报告 `E2E-AVM-016`（2026-09-16 首次实测）+ 生产 sched-1 日志（2026-09-23 前后
#   24h 内 38 次该错误 → 出口 502）。
#   ✅ **修法已实施（2026-09-24）**："480p" 表改 `[5, 10, 15, 20]`，`snap_duration()`
#      **就近吸附并留痕**：6/7→5、8/9→10、11/12→10、13/14→15、16/17→15、18/19→20。
#   ⚠️ 吸附不会把免费区请求带进计费区：≤10s 的请求只落 5 或 10（均为免费档）；
#      11/12s 回到 10s（原请求已在计费区）——"出一档相近的片"优先于整单失败（2026-09-18 口径）。
#   ⚠️ **1080p 是否同为离散档位尚未实测**，保持 `range(5, 21)`，别顺手一起改。
#
...
#   ⚠️ 上述"连续"结论**只对 720p 成立**；480p 已按实测改为离散表（见顶）。
#
# ⚠️ 曾经把 480p 写成 `[5, 10, 15, 20]` —— **【2026-09-24 终审：那正是站点真行为，本条
#    历史疑虑作废（10s 本就在免费线内，8s→10s 不构成"跨进计费区"）；以下仅存史】**。
#    原文如下：那会让 `480p/8s`（本就在免费线内）被就近吸附到 **10s**，从而**跨进计费区**。
#    这类"静默改档 + 变贵"正是适配层最该避免的。已按站点真实行为改成连续区间。
...
#   免费档统一为 **≤10s 的秒值**：480p 即 5 与 10 两档；720p/1080p 为 5~10 任意整数秒。
#   ⚠️ 「480p 只有 5/10/15/20」**就是站点真行为** —— **【2026-09-24 终审：已按此实施
#      （`DURATION_ALLOWED["480p"] = [5,10,15,20]`）；E2E-AVM-016 + 生产 38 次 502 双重
#      实锤。以下原文作废，仅存史】**。原文：那是已被推翻的档位表（见上）。照它吸附会把
#      6s 拉成 5s 或 10s（静默改时长）、把 11s 拉成 10s（跨回免费区），两者都属「不报错、
#      但结果不是调用方要的」。
...
def snap_duration(duration: Any, resolution: str, prefer_free: bool = False) -> int:
    ...
    `720p`/`1080p` 的约束是"连续秒数 + 上限"，绝大多数请求**原样透传**，只有越界值
    才会被就近钳制；**`480p` 是离散表 `[5, 10, 15, 20]`**（站点实测），非档位值一律
    就近吸附 —— 不吸附的话 6/7/8/9/11..14/16..19s 会原样发出并 502。

    `prefer_free=True` 是**省钱开关**，语义为"别越过免费线"：凡是超过
    `FREE_MAX_DURATION` 的合法时长（例如 15s）都会被主动拉回免费上界 —— 而不只是
    在越界时才生效。

    吸附由调用侧留痕（`duration {requested}s snapped to {snapped}s` 告警）。
    """
...
def translate_create(
    ...
    *,
    ...
    if requested_duration is not None and requested_duration != duration:
        ...
        #    `[5, 20]` 连续区间下吸附只在越界时发生，因此
        #    `duration > FREE_MAX_DURATION >= requested_duration` 永不可能成立
        #    （2026-09-14 报告 AVM12-OPEN-DEAD 实测确认：`grep` 查得到代码、但跑不到）。
        #    2026-09-24 480p 改离散表后**仍不可达**：≤10s 的请求只落 5 或 10（都仍在免费线内），
        #    能越过免费线的只有 15/20（原请求必 >10s ⇒ 前提不成立）。
