...
function stripQuotes(v) {
  ...
  const q = t[0];
  if (t.length >= 2 && (q === "'" || q === '"' || q === '`') && t[t.length - 1] === q) {
    return t.slice(1, -1).trim();
  }
  return t;
}

// Set-Cookie *attributes*.  They are never valid in a Cookie *request* header, so
// they must be dropped when someone pastes a whole `Set-Cookie:` response line.
// (`HttpOnly` / `Secure` / `Partitioned` carry no `=` and are dropped as blanks.)
...
/**
 * Normalise whatever the user pasted into a real `Cookie` header value.
 *
 * The value is sent VERBATIM as the `Cookie` request header, so a malformed one
 * does not fail loudly -- the server just treats you as logged out, which is
 * indistinguishable from an expired session.  So accept the forms people actually
 * paste, and say so out loud whenever we had to repair one.
 *
 *   'abc…'                           (bare token) -> 'auth_session=abc…' + warn
 *   'auth_session=abc…'                           -> unchanged
 *   'auth_session=abc…; NEXT_LOCALE=zh'           -> unchanged
 *   'Cookie: auth_session=abc…; NEXT_LOCALE=zh'   -> label stripped
 *   "-H 'Cookie: auth_session=abc…'"              -> cURL wrapper stripped
 *   'Set-Cookie: auth_session=abc…; Path=/; HttpOnly' -> attributes dropped
 *   '[{"name":"auth_session","value":"abc…"}]'    -> jar JSON joined
 *
 * Anything unrecognisable is passed through **untouched** so the server, not this
 * function, produces the error.  Never guess.
 */
export function normalizeCookieHeader(raw) {
  let s = stripQuotes(raw ?? '');

  // A whole exported cookie jar, pasted as JSON
  if (s.startsWith('[')) {
    try {
      const jar = JSON.parse(s);
    ...
    } catch { /* not JSON after all -- keep going with the raw text */ }
  }

  // Peel shell / header noise until it stops changing (bounded; always shrinks).
  // A loop because the layers nest: -H 'Cookie: …' needs two passes.
  for (let i = 0; i < 5; i++) {
    const before = s;
    s = stripQuotes(s)
      .replace(/^(?:-H|--header)\s+/i, '')
      .replace(/^(?:--cookie|-b)\s+/i, '')
      .replace(/^(?:set-)?cookie\s*:\s*/i, '');
    s = stripQuotes(s);
    if (s === before) break;
  }
  // Deliberately NO global whitespace collapse: a multi-line paste where every line
  // ends in `;` already parses via the per-segment trim below, while a missing `;`
  // is genuinely malformed and must go out AS-IS so the HTTP layer fails loudly --
  // collapsing it would silently weld two pairs into one and send a wrong cookie.
  ...
  if (s.includes('=')) {
    const parts = s
      ...
      .filter((p) => {
        const eq = p.indexOf('=');
        if (eq < 0) return false; // bare 'HttpOnly' / 'Secure'
        return !SET_COOKIE_ATTRS.has(p.slice(0, eq).trim().toLowerCase());
      });
    return parts.join('; ');
  }

  // No '=' at all -- either a bare token, or something we refuse to guess at
  if (/^[A-Za-z0-9._-]{16,}$/.test(s)) {
    console.warn('[cookie] AVM_COOKIE looks like a bare token, not a Cookie header.');
    console.warn('[cookie] expanding it to "auth_session=<token>" for you.');
...
export class AvmClient {
   ...
   */
  constructor(opts = {}) {
    this.cookie = normalizeCookieHeader(opts.cookie || process.env.AVM_COOKIE || '');
